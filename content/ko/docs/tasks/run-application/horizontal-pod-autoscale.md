...
비율이다. 사용률 계산 및 평균 산출 방법에 대한 자세한 내용은 [알고리즘](#algorithm-details)
을 참조한다.
...
해당 파드는 무시되고 권장 사항이 다시 계산된다. 계산에 대한 자세한 내용은 [알고리즘](#algorithm-details)
...
효율적으로 유지할 수 있다. 컨테이너를 정의하는 리소스 (예: 배포)
를 업데이트하기 전에 연결된 HPA를 업데이트하여 새 컨테이너 이름과 이전 컨테이너 이름을
